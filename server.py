...
@app.post("/translate")
async def translate_voice(request: Request, file: UploadFile = File(...)):
      try:
            ...
            # Speak translated query
            speech_synth = speechsdk.SpeechSynthesizer(speech_config=speech_config)
            speech_synth.speak_text_async(translated_query).get()
            time.sleep(1)
            ...
            translated_response = reverse_translation.choices[0].message.content.strip()

            # Speak translated response
            speech_config.speech_synthesis_voice_name = get_tts_voice(source_lang)
            speech_synth = speechsdk.SpeechSynthesizer(speech_config=speech_config)
            speech_synth.speak_text_async(translated_response).get()
            time.sleep(1)
